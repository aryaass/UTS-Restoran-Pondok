<?php 
	...
	$user = 0;
	if(isset($_COOKIE['isLoged'])) {
		$user = getUserById($_COOKIE['isLoged']);
	}
	else echo '<script>alert("Please Login first before checkout :)")</script>';
	...
	if (!isset($_SESSION["cart_array"]) || count($_SESSION["cart_array"]) < 1) {
		...
		foreach ($_SESSION["cart_array"] as $each_item) { 
			...
			$cartTotal = (int) $pricetotal + $cartTotal;
			// var_dump($cartTotal, $price, $each_item['quantity']);
		...
		$cartTotal = '<p class="p_total"><span>Basket Total</span> : Rp<span id="cart-total">'.$cartTotal.'</span></p>';	
	}
?>
...
<head>
	<title>Restoran Pondok | Order</title>
	...
	<div class="info_holder">
		...
		<form method="post" action="" onSubmit="validate_input(); return false">
			<div class="form_group">
				<div class="form_group">
					<label>Name</label>
					<input type="text" id="name" name="name" placeholder="Enter your full name" value="<?=$user['username'];?>" required>
				</div>
				<div class="form_group">
					<label>Address</label>
